...
public class ZRange implements Cloneable, HasSize, HasPermute<ZRange>, HasJsonOutput {
     ...
     */
    @Nonnull
    public ZRange embed(@Nonnull ZRange embedRange) {
        int baseNDim = getNDim();
        int embedNDim = embedRange.getNDim();
        ZPoint rStart = start.addDims(0, embedNDim)
                .add(embedRange.start.addDims(-1, baseNDim));
        ZPoint rEnd = end.addDims(0, embedNDim)
                .add(embedRange.end.addDims(-1, baseNDim));
     ...
     */
    @Nonnull
    public ZRange embed(@Nonnull ZTensorWrapper shape) {
        return embed(ZRange.newFromShape(shape));
    }
}
