...
public final class ZTensor
    ...
    public ZTensor select(@Nonnull List<Selector> selectors) {
        ...
        for (var selIdx = 0; selIdx < selectors.size(); selIdx++) {
            ...
            switch (selector) {
                case Index index ->
                        cur = cur.selectDim(nextDim, index.getIndex()); // shrinks cur, so we don't increment nextDim.
     ...
     */
    @Nonnull
    public ZTensor sliceDim(int dim, @Nullable Integer start, @Nullable Integer end, @Nullable Integer step) {
        var d = resolveDim(dim);
        ...
        end = IndexingFns.resolveIndex("end", end, shape[d], shape[d] + 1);
        ...
        // FIXME - slicing is more complex than this.
