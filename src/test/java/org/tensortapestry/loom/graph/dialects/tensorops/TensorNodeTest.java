...
public class TensorNodeTest extends BaseTestClass {
  ...
  public void test_scalar_body() {
    var body = TensorBody.builder().dtype("int32").shape(ZPoint.of()).build();
    assertJsonEquals(
      body,
      """
            {
  ...
  public void test_body() {
    ...
    assertThat(body).hasToString("TensorBody(dtype=int32, range=zr[-1:2, -1:3])");
  }

  @Test
  public void test_body_builder() {
    {
      var body = TensorBody.builder().dtype("int32").shape(ZPoint.of(2, 3)).build();
      assertThat(body.getRange()).isEqualTo(ZRange.newFromShape(2, 3));
    }
    {
      var body = TensorBody.builder().dtype("int32").shape(2, 3).build();
      assertThat(body.getRange()).isEqualTo(ZRange.newFromShape(2, 3));
    }
    {
      var body = TensorBody.builder().dtype("int32").shape(ZTensor.newVector(2, 3)).build();
      assertThat(body.getRange()).isEqualTo(ZRange.newFromShape(2, 3));
    }
  }
}
