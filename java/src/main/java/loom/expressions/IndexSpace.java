...
@Jacksonized
@Builder
public final class IndexSpace implements HasDimension, HasToJsonString {
  ...
  public static IndexSpace parseIndexSpace(String str) {
    if (str.startsWith("{")) {
      return JsonUtil.fromJson(str, IndexSpace.class);
    }

    if (str.startsWith("i[") && str.endsWith("]")) {
      var t = str.substring(2, str.length() - 1).trim();
      if (t.isEmpty()) {
        return new IndexSpace(
