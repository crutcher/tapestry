...
public class TGraphTest implements CommonAssertions {
  ...
  public void test_query_tags() {
    ...
    var barrier = graph.addNode(new THappensAfterEdge(sp2.id, sp1.id));

    assertThat(graph.queryTags(THappensAfterEdge.class).toSingleton()).isSameAs(barrier);
  }
  ...
  public void test_validate() {
    ...
    assertThat(graph.queryEdges(THappensAfterEdge.class).withTargetId(sp1.id).toSingleton())
        .isSameAs(barrier);
    ...
    var simple = graph.addNode(new SimpleNode());
    graph.addNode(new THappensAfterEdge(simple.id, simple.id));
